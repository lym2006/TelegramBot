...
4. **首次启动**：双击 `TelegramBot\TelegramBot.exe`，确认后自动安装运行环境（嵌入式 Python、依赖、浏览器内核，默认走国内镜像加速），进度窗口实时显示安装步骤与下载速率，期间保持网络畅通。
...
- **代理语义**：配置地址优先尝试；留空或试不通即自动降级（系统代理 → 直连，TUN 由系统层透明接管、归入直连）。三级全不通时才会扫描本机常用代理端口并**提示**可填地址，需你确认后才写入，程序绝不自动采用。代理软件只关系统代理开关、进程仍在监听时，程序仍可直连其本地端口收发消息，属正常现象而非缓存故障。
